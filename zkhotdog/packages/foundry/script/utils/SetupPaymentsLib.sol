...
library SetupPaymentsLib {
    ...
    function createAVSRewardsSubmissions(
        ...
        ECDSAServiceManagerBase(zkHotdogServiceManager).createAVSRewardsSubmission(
            rewardsSubmissions
        );
    }
    ...
    function createOperatorDirectedAVSRewardsSubmissions(
        ...
        for (uint256 i = 0; i < numPayments; i++) {
            ...
            IRewardsCoordinator.OperatorDirectedRewardsSubmission({
                ...
                startTimestamp: startTimestamp,
                duration: duration,
                description: "zkHotdog verification rewards"
            });

            rewardsSubmissions[i] = rewardsSubmission;
        }
        ECDSAServiceManagerBase(zkHotdogServiceManager).createOperatorDirectedAVSRewardsSubmission(
            rewardsSubmissions
        );
    }
    ...
    function processClaim(
        ...
        // This workflow assumes a new root submitted for every payment claimed. So we get the latest root index to process a claim for
